...
給開發與維護看的技術細節。對外的介紹在 [README](../README.md)，設計決策的來龍去脈在[個案研究](個案研究.md)。

這個專案是 `Works Page Hi-Fi.dc.html` 的實作。那份高擬真稿其實不只作品集列表，
...
部署在 Cloudflare Workers（<https://margin-salon.momoopsoops.workers.dev/>），
GitHub 的 `main` 有新 commit 就自動建置部署（Workers Builds）。

- Worker 的設定在 `wrangler.jsonc`：`nodejs_compat`、靜態檔輸出、流量限制認的 `cf-connecting-ip`。
  後台的 Worker 名稱要跟裡面的 `name` 一樣，不然建置會失敗。
- 公開站跑**示範模式**：後台不設 Google 日曆與 Resend 的任何變數，訪客試約不會進真的日曆、也不寄信。
- **限流在 Workers 上擋不住。** 計數記在記憶體裡，Workers 會同時開很多個 isolate 各算各的。
  2026-09-15 對線上站連送 22 次應徵 API（上限每小時 5 次），只有 2 次回 429。
  示範模式下影響很小；要接真的服務之前，先換成 Workers 的 Rate Limiting 綁定，
  或綁自己的網域再設 Cloudflare 的 Rate limiting rules（免費方案 1 條、以 IP 計）。
- **接真的日曆之前還沒驗證的兩件事**：
  1. `nodejs_compat` 下 `createSign` 能不能簽出 Google 的 JWT（沒開時 Nitro 會把它換成「未實作」的替身）。
  2. `server/utils/gcal.ts` 與 `mail.ts` 用不帶 `event` 的 `useRuntimeConfig()`，
     它回傳的是模組載入時算好的設定；Cloudflare 的環境變數跟著請求進來，金鑰可能讀不到、安靜退回示範模式。
...
6. 部署在平台或反向代理後面時，要填 `NUXT_CLIENT_IP_HEADER`。Cloudflare Workers 已經在
   `wrangler.jsonc` 設成 `cf-connecting-ip`；其他平台照 `.env.example` 的對照填。
